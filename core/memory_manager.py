# =============================================================================
# LLM2POR Autonomous System - Memory Manager
# =============================================================================
...
class ExperimentLogger:
    ...
    def __init__(self, experiment_dir: str):
        ...
        self._write_log(f"LLM2POR EXPERIMENT LOG\n")
    ...
    def log_matchmaker_results(self, results: Dict[str, Any]):
        """Log matchmaker results (PORMAKE or QMOF)."""
        self._write_log(f"--- MATCHMAKER RESULTS ---\n")
        if isinstance(results, dict):
            if results.get('qmof_mode'):
                q = len(results.get('qmof_ids', []))
                self._write_log(f"Mode:        QMOF (Direct Matching)\n")
                self._write_log(f"Matches:     {q}\n")
            else:
                ...
                self._write_log(f"Total Space: {total_combos:,} combinations\n")
        else:
...
def test_memory_and_logger():
    ...
    test_dir = tempfile.mkdtemp(prefix="llm2por_test_")
    ...
    memory.add_iteration(
        ...
        feedback_type="3-Beam Diagnostic",
