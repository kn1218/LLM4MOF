...
def collect_results(
    ...
    # Load geometry_filter from batch_manifest.json (one level up from results dir)
    geometry_filter: Optional[dict] = None
    manifest_path = os.path.join(
        os.path.dirname(os.path.dirname(results_path)), "batch_manifest.json"
    )
    if os.path.exists(manifest_path):
        with open(manifest_path, "r", encoding="utf-8") as f:
            manifest = json.load(f)
        geometry_filter = manifest.get("config", {}).get("geometry_filter")
    ...
    for beam_id in ["Z", "A", "F", "total"]:
        ...
        for r in results_list:
            ...
            # Compute geo_filter_passed from real Zeo++ geometry (strict LLM range)
            geo_filter_passed = True
            geo_filter_fail_reason = ""
            if real_geometry is not None and geometry_filter:
                fails = []
                for feat in ("di", "df", "vf", "sa", "cv", "density", "dif"):
                    val = real_geometry.get(feat)
                    if val is None:
                        continue
                    for suffix, fn in (("_min", lambda v, b: v < b), ("_max", lambda v, b: v > b)):
                        for key in (f"target_{feat}{suffix}", f"target_{feat.capitalize()}{suffix}"):
                            bound = geometry_filter.get(key)
                            if bound is not None and fn(val, bound):
                                direction = "< min" if suffix == "_min" else "> max"
                                fails.append(f"{feat}={val:.3f} {direction} {bound}")
                if fails:
                    geo_filter_passed = False
                    geo_filter_fail_reason = "; ".join(fails)
            ...
            sim_result = SimResult(
                ...
                geo_filter_passed=geo_filter_passed,
                geo_filter_fail_reason=geo_filter_fail_reason,
            )
