# =============================================================================
# LLM4MOF Autonomous System - Sensitivity Analyzer
# =============================================================================
# Ported from the project's pilot prototype (Block 3)
...
class SensitivityAnalyzer:
        ...
        try:
            ...
            if self.is_hmof:
                ...
                if target_col in self.df_master.columns:
                    ...
                    print(f"[ERROR] hMOF DB: column '{target_col}' not found — "
                          f"EF/enrichment calculations will be SKIPPED. "
                          f"Available: {list(self.df_master.columns)[:10]}...")
            elif self.is_qmof:
    ...
    def run_analysis(self, agent2_output: dict, matchmaker_results: dict, 
        ...
        SA_MIN, SA_MAX = get_lims('target_sa_min', 'target_sa_max')  # Surface Area (m²/cm³)
        ...
        print(f"   SA: [{SA_MIN:.0f}, {SA_MAX:.0f}] m²/cm³ | VF: [{VF_MIN:.2f}, {VF_MAX:.2f}]")
        ...
        # Check direct-MOF-filtering modes (QMOF, hMOF). These match whole MOFs from a
        # fixed database (no topology x node x edge assembly), so they report a flat candidate
        # count, NOT a PORMAKE-style N x L x T design space.
        is_qmof = config.is_qmof_mode()
        is_hmof = config.is_hmof_mode()

        if is_qmof:
            print(f"║   QMOFs Matching Chemistry:    {len(set_a):<8}                 ║")
            print(f"╠══════════════════════════════════════════════════════════╣")
            print(f"║   SEARCH SPACE:        {len(set_a):<12} candidates      ║")
            print(f"║   (Direct database matching)                              ║")
        elif is_hmof:
            print(f"║   hMOFs Matching Chemistry:    {len(set_a):<8}                 ║")
            print(f"╠══════════════════════════════════════════════════════════╣")
            print(f"║   SEARCH SPACE:        {len(set_a):<12} candidates      ║")
            print(f"║   (Direct database matching)                              ║")
        else:
