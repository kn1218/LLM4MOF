...
def create_raspa_input(mof, mof_dir, output_dir, params):
    ...
    forcefield_path = os.path.abspath(
        params.get("forcefield_dir", DEFAULT_FORCEFIELD_DIR)
    )
    ...
    simulation_json = {
        ...
        "Components": [
            {
                "Name": "hydrogen",
                "MoleculeDefinition": os.path.join(forcefield_path, "hydrogen.json"),
...
def main():
    ...
    parser.add_argument(
        "--forcefield-dir",
        default=DEFAULT_FORCEFIELD_DIR,
        help="Force field directory (JSON format)",
    )
    parser.add_argument("--temperature", type=float, default=77, help="Temperature (K)")
    parser.add_argument("--pressure", type=float, default=5e5, help="Pressure (Pa)")
    parser.add_argument(
    ...
    print(f"    Pressure: {args.pressure} Pa ({args.pressure / 1e5:.1f} bar)")
    print(f"    Cutoff: {args.cutoff} Angstrom")
    print(f"    Forcefield: {args.forcefield_dir}")
