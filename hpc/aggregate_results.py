    ...
    for job in manifest["jobs"]:
        ...
        if os.path.exists(result_file):
            with open(result_file, "r", encoding="utf-8") as f:
                result = json.load(f)
            # Carry forward manifest-level flags not present in per-MOF result JSON
            if "geo_filter_passed" not in result and "geo_filter_passed" in job:
                result["geo_filter_passed"] = job["geo_filter_passed"]
            if "geo_filter_fail_reason" not in result and "geo_filter_fail_reason" in job:
                result["geo_filter_fail_reason"] = job["geo_filter_fail_reason"]
            results.append(result)
            if result.get("status") in ("success", "stage1_done"):
                n_success += 1
            else:
                n_fail += 1
        else:
            ...
            results.append({
                ...
                "geo_filter_passed": job.get("geo_filter_passed", True),
                "geo_filter_fail_reason": job.get("geo_filter_fail_reason", ""),
                "status": "missing",
                "real_uptake": None,
