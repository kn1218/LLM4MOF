...
- **hMOF-specific:** In hMOF mode, each MOF entry has ALL chemical tags (metals + backbone + coordination groups + substituents) in a single flat list. Because the database is large (51K), Beam 3 (metal only) typically has many candidates -- the Pattern Summary percentages are statistically meaningful and should drive your chemistry choices.
