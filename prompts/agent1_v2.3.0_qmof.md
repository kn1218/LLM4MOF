...
### Rule A: Extract Chemistry Patterns, Not Individuals
When reading Beam 3 (Chemistry Distribution) feedback, **never anchor on a single high-performing structure**. Instead:
- Read the **Pattern Summary** first. Which metals appear at >20% frequency? Which backbones dominate? What oxidation states recur?
- For electronic properties, **chemistry patterns are the primary signal**. Pay close attention to: which metal families dominate (d-block vs main-group), which linker backbone types recur (conjugated vs non-conjugated), and whether specific functional groups correlate with performance.
- Ask: "What electronic mechanism do the top performers SHARE?" Not: "What is the best single structure?"
- A metal appearing once at rank #1 is an anecdote. A backbone appearing in 60% of top-10 is a pattern.

**ANTI-PATTERN (FORBIDDEN):** "Beam 3 shows Cu+thiophene at bandgap 0.3 eV, so I will use Cu+thiophene." This copies one data point. Instead: "Beam 3 shows late transition metals (Cu 25%, Co 20%) and conjugated backbones (thiophene 40%, pyridine 30%) dominating. The mechanism may be metal-to-ligand charge transfer with extended conjugation lowering the HOMO-LUMO gap."

### Rule B: Hypothesis Falsification (Scientific Method)
Each iteration should **test a specific electronic mechanism**, not just chase performance. Structure your reasoning as:
...
- **Prediction:** "I expect performance in range [N, M] because..."

Examples of good electronic hypotheses:
- "If conjugation length drives bandgap narrowing, then extending the linker backbone from single-ring to multi-ring should red-shift the gap."
- "If d-electron count controls bandgap, then switching from d0 metals to d6 metals should change the gap significantly."
- "If metal-ligand charge transfer dominates, then electron-rich linkers paired with reducible metals should yield the narrowest gaps."
...
- **Iterations 1-2:** Broad exploration. Test 2-3 fundamentally different metal/ligand chemistry families. Cast a wide net with minimal geometric constraints. The goal is INFORMATION about which electronic mechanisms matter, not peak performance.
- **Iterations 3-4:** Focused exploitation. Double down on the most promising electronic mechanism from iterations 1-2. Refine the chemistry (specific oxidation states, functional group combinations).
...
- Include 3-4 diverse branches covering different electronic mechanisms
...
- **Geometry:** Only constrain descriptors that are mechanistically relevant to your target property. For electronic properties, avoid constraining geometric descriptors (Di, Df, SA, VF) unless you have strong empirical evidence from previous iterations that geometry matters. Start with chemistry-only constraints.
...
### Rule G: Electronic Property Awareness
When the target property is electronic in nature (bandgap, conductivity, electronic energy), keep these principles in mind:
- **Primary levers for electronic properties:** metal d-electron configuration, ligand HOMO-LUMO gap (controlled by conjugation length and electron-withdrawing/donating substituents), and the nature of metal-ligand bonding (charge transfer character).
- **Geometric descriptors are generally secondary:** Pore size (Di, Df), surface area (SA), void fraction (VF), and density are structural descriptors that govern gas adsorption and molecular transport. They have little direct influence on electronic band structure. Do NOT waste iterations constraining these for electronic property optimization unless feedback explicitly shows geometry matters.
- **QMOF-specific electronic descriptors are your strongest tools:** `oxidation_states` and `coordination_geometry` directly control crystal field splitting and band structure. Use these to test electronic hypotheses. `has_open_metal_sites` affects local electronic density of states.
- **Conjugation is a linker-side lever:** The degree of pi-conjugation in the organic linker controls the ligand-centered HOMO-LUMO gap. Longer conjugated backbones generally narrow this gap; electron-withdrawing groups can widen or shift it.
- **Do not assume which direction is "better":** Whether a narrower or wider gap is desired depends entirely on the user inquiry. Keep your reasoning general and let the target guide exploitation direction.
...
* **Performance (The Goal - Start Here):**
    * `target` -- The primary performance metric for this experiment (e.g., bandgap in eV for electronic property searches). This is the ONLY metric the database can evaluate.
...
```JSON
{
  "meta_cognition": {
      ...
      "hypothesis_to_test": "State the specific mechanism you are testing in this iteration (e.g., 'Extended linker conjugation narrows bandgap more than metal identity')",
